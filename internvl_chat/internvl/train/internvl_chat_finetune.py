...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        # TODO: quick resume
        self._state_dict = {}
        self.task_desc_root = task_desc_root  # 数据集根路径
    ...
    def multi_modal_get_item(self, data_item):
        ...
        sample_folder = os.path.dirname(image_path)
        mask_path = os.path.join(sample_folder, f"{os.path.splitext(img_name)[0]}_mask.npy")
        # 加载mask
        if os.path.exists(mask_path):
            mask = torch.from_numpy(np.load(mask_path)).float().to(self.device)
            logger.info(f'Load mask: {mask_path}')
        else:
    ...
    def multi_modal_multi_image_get_item(self, data_item):
        ...
        for image_path in data_item['image']:
            ...
            if os.path.exists(mask_path):
                ...
                mask = torch.ones((1, 640, 512))  # Default mask if not found
            masks.append(mask)
        # Stack masks into a single tensor of shape (B, H, W)
        image_mask = torch.stack(masks).squeeze(1)  # Remove the extra dimension if masks are (1, H, W)
        pixel_values = [transform(image) for image in images]
        pixel_values = torch.stack(pixel_values)
        num_patches = pixel_values.size(0)
        ...
        image_end_token_id = self.tokenizer.convert_tokens_to_ids(IMG_END_TOKEN)
        # 在断言前打印结束标记统计
        image_end_count = (ret['input_ids'][0] == image_end_token_id).sum()
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        task_desc_root = ds_collections[ds_name]['root']
